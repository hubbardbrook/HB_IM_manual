...
The Mailman list service at UNH is no longer in operation. The only option for lists at UNH is through Outlook groups. This is a service that is optimized for internal communications and does not function well with a broader community like ours. The longstanding unresolved issue is with DMARC rules at some institutions that cause rejection of messages that do not originate from UNHan. For that reason, we do not have full function of two-way conversations on the list. Messages sent from a UNH address are delivered to all, so at the current time the IM forwards messages to the list. There weren't many interactive list conversations in the past, so this hasn't been a big issue, but it is annoying.
